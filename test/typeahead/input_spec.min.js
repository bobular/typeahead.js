describe("Input", function () {
    var b, c;
    b = {enter: 13, esc: 27, tab: 9, left: 37, right: 39, up: 38, down: 40, normal: 65};
    c = WWW();
    beforeEach(function () {
        var f;
        setFixtures(fixtures.html.input + fixtures.html.hint);
        f = $("#jasmine-fixtures");
        this.$input = f.find(".tt-input");
        this.$hint = f.find(".tt-hint");
        this.view = new Input({input: this.$input, hint: this.$hint}, c).bind()
    });
    ...
    describe("when the blur DOM event is triggered", function () {
        ...
        it("should trigger blurred", function () {
            var f;
            this.view.onSync("blurred", f = jasmine.createSpy());
            this.$input.blur();
    ...
    describe("when the focus DOM event is triggered", function () {
        ...
        it("should trigger focused", function () {
            var f;
            this.view.onSync("focused", f = jasmine.createSpy());
            this.$input.focus();
            expect(f).toHaveBeenCalled()
        })
    });
    describe("when the keydown DOM event is triggered by tab", function () {
        it("should trigger tabKeyed if no modifiers were pressed", function () {
            var f;
            this.view.onSync("tabKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.tab);
            expect(f).toHaveBeenCalled()
        });
        it("should not trigger tabKeyed if modifiers were pressed", function () {
            var f;
            this.view.onSync("tabKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.tab, true);
    ...
    describe("when the keydown DOM event is triggered by esc", function () {
        it("should trigger escKeyed", function () {
            var f;
            this.view.onSync("escKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.esc);
            expect(f).toHaveBeenCalled()
        })
    });
    describe("when the keydown DOM event is triggered by left", function () {
        it("should trigger leftKeyed", function () {
            var f;
            this.view.onSync("leftKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.left);
            expect(f).toHaveBeenCalled()
        })
    });
    describe("when the keydown DOM event is triggered by right", function () {
        it("should trigger rightKeyed", function () {
            var f;
            this.view.onSync("rightKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.right);
            expect(f).toHaveBeenCalled()
        })
    });
    describe("when the keydown DOM event is triggered by enter", function () {
        it("should trigger enterKeyed", function () {
            var f;
            this.view.onSync("enterKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.enter);
            expect(f).toHaveBeenCalled()
        })
    });
    ...
    describe("when the keydown DOM event is triggered by down", function () {
        it("should trigger downKeyed", function () {
            var f;
            this.view.onSync("downKeyed", f = jasmine.createSpy());
            e(this.$input, "keydown", b.down);
            expect(f).toHaveBeenCalled()
        });
        it("should prevent default if no modifers were pressed", function () {
            var f = e(this.$input, "keydown", b.down);
            expect(f.preventDefault).toHaveBeenCalled()
        });
        it("should not prevent default if modifers were pressed", function () {
            var f = e(this.$input, "keydown", b.down, true);
            expect(f.preventDefault).not.toHaveBeenCalled()
        })
    });
    describe("when the input DOM event is triggered", function () {
        ...
        it("should trigger queryChanged if the query changed", function () {
            ...
            this.view.setQuery("wine");
            this.view.setInputValue("cheese");
            this.view.onSync("queryChanged", f = jasmine.createSpy());
            a(this.$input);
            expect(f).toHaveBeenCalled()
        });
        it("should trigger whitespaceChanged if whitespace changed", function () {
            ...
            this.view.onSync("whitespaceChanged", f = jasmine.createSpy());
            a(this.$input);
            expect(f).toHaveBeenCalled()
        });
        ...
        it("should check lang direction", function () {
            ...
            this.view.onSync("langDirChanged", f = jasmine.createSpy());
            a(this.$input);
    ...
    describe("#hasOverflow", function () {
        it("should return true if the input has overflow text", function () {
            var f = new Array(1000).join("a");
            this.view.setInputValue(f);
    ...
    describe("#destroy", function () {
        it("should remove event handlers", function () {
            ...
            spyOn(g, "off");
            this.view.destroy();
            expect(f.off).toHaveBeenCalledWith(".tt");
            expect(g.off).toHaveBeenCalledWith(".tt")
        });
        it("should set references to DOM elements to dummy element", function () {
            var f, h, g;
            f = this.view.$hint;
            h = this.view.$input;
            g = this.view.$overflowHelper;
            this.view.destroy();
            expect(this.view.$hint).not.toBe(f);
            expect(this.view.$input).not.toBe(h);
            expect(this.view.$overflowHelper).not.toBe(g)
        })
    });
    function a(f) {
        var g, h;
        h = _.isMsie() ? "keypress" : "input";
        g = $.Event(h);
        f.trigger(g)
    }
    ...
    function e(f, i, h, j) {
        var g;
        g = $.Event(i, {keyCode: h, altKey: !!j, ctrlKey: !!j, metaKey: !!j, shiftKey: !!j});
        spyOn(g, "preventDefault");
        f.trigger(g);
        return g
    }

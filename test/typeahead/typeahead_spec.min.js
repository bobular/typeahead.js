describe("Typeahead", function () {
    ...
    beforeEach(function () {
        ...
        d = $("#jasmine-fixtures");
        this.$input = d.find("input");
        b = {val: "foo bar", obj: "fiz"};
        this.view = new Typeahead({input: new Input(), menu: new Menu(), eventBus: new EventBus({el: this.$input})}, c);
    ...
    describe("on selectableClicked", function () {
        ...
        describe("when active", function () {
            ...
            it("should select the selectable", function () {
                ...
                this.menu.trigger(d, e);
                expect(this.view.select).toHaveBeenCalledWith(e)
            })
        })
    });
    describe("on asyncRequested", function () {
        ...
        it("should trigger typeahead:asyncrequest", function () {
            var e = jasmine.createSpy();
            this.$input.on("typeahead:asyncrequest", e);
            this.menu.trigger(d);
    ...
    describe("on asyncCanceled", function () {
        ...
        it("should trigger typeahead:asynccancel", function () {
            var e = jasmine.createSpy();
            this.$input.on("typeahead:asynccancel", e);
            this.menu.trigger(d);
            expect(e).toHaveBeenCalled()
        })
    });
    describe("on asyncReceived", function () {
        ...
        it("should trigger typeahead:asyncreceive", function () {
            var e = jasmine.createSpy();
            this.$input.on("typeahead:asyncreceive", e);
            this.menu.trigger(d);
            expect(e).toHaveBeenCalled()
        })
    });
    describe("on datasetRendered", function () {
        ...
        describe("when active", function () {
            ...
            it("should update the hint", function () {
                ...
                this.menu.getSelectableData.andReturn(b);
                this.input.getInputValue.andReturn(b.val.slice(0, 2));
                this.menu.trigger(d);
                expect(this.input.setHint).toHaveBeenCalled()
            });
            it("should trigger typeahead:render", function () {
                var e = jasmine.createSpy();
                this.$input.on("typeahead:render", e);
                this.menu.trigger(d);
                expect(e).toHaveBeenCalled()
            })
        })
    ...
    describe("on blurred", function () {
        ...
        it("should trigger typeahead:change if query changed since focus", function () {
            ...
            this.$input.on("typeahead:change", e);
            this.input.trigger(d);
            expect(e).toHaveBeenCalled()
        });
    ...
    describe("on enterKeyed", function () {
        ...
        describe("when active and menu is open", function () {
            ...
            it("should select selectable if there is an active one", function () {
                ...
                f = $("<bah>");
                spyOn(this.view, "select");
                this.menu.getActiveSelectable.andReturn(f);
                this.input.trigger(d, e);
                expect(this.view.select).toHaveBeenCalledWith(f)
            });
            ...
            it("should not select selectable if there is no active one", function () {
                ...
                spyOn(this.view, "select");
                this.input.trigger(d, e);
                expect(this.view.select).not.toHaveBeenCalledWith(f)
            });
    ...
    describe("on tabKeyed", function () {
        ...
        describe("when active and menu is open", function () {
            ...
            it("should autocomplete to top suggestion", function () {
                ...
                this.menu.getTopSelectable.andReturn(f);
                this.input.trigger(d, e);
                expect(this.view.autocomplete).toHaveBeenCalledWith(f)
            });
    ...
    describe("on whitespaceChanged", function () {
        ...
        describe("when active", function () {
            ...
            it("should update the hint", function () {
                ...
                this.input.getInputValue.andReturn(b.val.slice(0, 2));
                this.input.trigger(d, e);
                expect(this.input.setHint).toHaveBeenCalledWith(b.val)
            })
        })
    });
    ...
    describe("#activate", function () {
        describe("when active", function () {
            ...
            it("should do nothing", function () {
                ...
                this.$input.on("typeahead:beforeactive", d);
                this.view.activate();
        ...
        describe("when idle and enabled", function () {
            ...
            it("should support cancellation", function () {
                var e, d;
                e = jasmine.createSpy().andCallFake(a);
                d = jasmine.createSpy();
                this.$input.on("typeahead:beforeactive", e);
                this.$input.on("typeahead:active", d);
                this.view.activate();
                expect(e).toHaveBeenCalled();
                expect(d).not.toHaveBeenCalled()
            });
    ...
    describe("#deactivate", function () {
        describe("when idle", function () {
            ...
            it("should do nothing", function () {
                var d = jasmine.createSpy();
                this.$input.on("typeahead:beforeidle", d);
                this.view.deactivate();
                expect(d).not.toHaveBeenCalled()
            })
        });
        describe("when active", function () {
            ...
            it("should support cancellation", function () {
                ...
                e = jasmine.createSpy().andCallFake(a);
                d = jasmine.createSpy();
                this.$input.on("typeahead:beforeidle", e);
                this.$input.on("typeahead:idle", d);
                this.view.deactivate();
                expect(e).toHaveBeenCalled();
                expect(d).not.toHaveBeenCalled()
            });
    ...
    describe("#open", function () {
        describe("when open", function () {
            ...
            it("should do nothing", function () {
                var d = jasmine.createSpy();
                this.$input.on("typeahead:beforeopen", d);
                this.view.open();
                expect(d).not.toHaveBeenCalled()
            })
        });
        describe("when closed", function () {
            ...
            it("should support cancellation", function () {
                ...
                e = jasmine.createSpy().andCallFake(a);
                d = jasmine.createSpy();
                this.$input.on("typeahead:beforeopen", e);
                this.$input.on("typeahead:open", d);
                this.view.open();
                expect(e).toHaveBeenCalled();
                expect(d).not.toHaveBeenCalled()
            });
    ...
    describe("#close", function () {
        describe("when closed", function () {
            ...
            it("should do nothing", function () {
                var d = jasmine.createSpy();
                this.$input.on("typeahead:beforeclose", d);
                this.view.open();
                expect(d).not.toHaveBeenCalled()
            })
        });
        describe("when open", function () {
            ...
            it("should support cancellation", function () {
                ...
                e = jasmine.createSpy().andCallFake(a);
                d = jasmine.createSpy();
                this.$input.on("typeahead:beforeclose", e);
                this.$input.on("typeahead:close", d);
                this.view.close();
                expect(e).toHaveBeenCalled();
                expect(d).not.toHaveBeenCalled()
            });
    ...
    describe("#select", function () {
        it("should do nothing if element is not a selectable", function () {
            ...
            this.$input.on("typeahead:beforeselect", d = jasmine.createSpy());
        ...
        it("should support cancellation", function () {
            ...
            d = jasmine.createSpy();
            this.menu.getSelectableData.andReturn(b);
            this.$input.on("typeahead:beforeselect", e).on("typeahead:select", d);
            this.view.select($("<bah>"));
            expect(e).toHaveBeenCalled();
            expect(d).not.toHaveBeenCalled()
        });
        it("should update query", function () {
            this.menu.getSelectableData.andReturn(b);
            this.view.select($("<bah>"));
            expect(this.input.setQuery).toHaveBeenCalledWith(b.val, true)
        });
        it("should trigger typeahead:select", function () {
            var d;
            this.menu.getSelectableData.andReturn(b);
            this.$input.on("typeahead:select", d = jasmine.createSpy());
            this.view.select($("<bah>"));
            expect(d).toHaveBeenCalled()
        });
        it("should close", function () {
    ...
    describe("#autocomplete", function () {
        it("should abort if the query matches the top suggestion", function () {
            var d;
            this.input.getQuery.andReturn(b.val);
            this.menu.getSelectableData.andReturn(b);
            this.$input.on("typeahead:beforeautocomplete", d = jasmine.createSpy());
        ...
        it("should support cancellation", function () {
            ...
            e = jasmine.createSpy().andCallFake(a);
            d = jasmine.createSpy();
            this.$input.on("typeahead:beforeautocomplete", e);
            this.$input.on("typeahead:autocomplete", d);
            this.menu.getSelectableData.andReturn(b);
            this.view.autocomplete($("<bah>"));
            expect(e).toHaveBeenCalled();
            expect(d).not.toHaveBeenCalled()
        ...
        it("should update the query", function () {
            this.menu.getSelectableData.andReturn(b);
            this.view.autocomplete($("<bah>"));
            expect(this.input.setQuery).toHaveBeenCalledWith(b.val)
        });
        it("should trigger typeahead:autocomplete", function () {
            var d;
            this.menu.getSelectableData.andReturn(b);
            this.$input.on("typeahead:autocomplete", d = jasmine.createSpy());
            this.view.autocomplete($("<bah>"));
            expect(d).toHaveBeenCalled()
        })
    });
    describe("#moveCursor", function () {
        ...
        it("should move cursor if minLength is not satisfied", function () {
            ...
            this.$input.on("typeahead:beforecursorchange", d);
        ...
        it("should support cancellation", function () {
            var d = jasmine.createSpy().andCallFake(a);
            this.$input.on("typeahead:beforecursorchange", d);
            this.view.moveCursor(1);
        ...
        it("should update the input value if moved to selectable", function () {
            this.menu.getSelectableData.andReturn(b);
            this.view.moveCursor(1);
            expect(this.input.setInputValue).toHaveBeenCalledWith(b.val)
        });
        ...
        it("should update the hint", function () {
            this.input.hasOverflow.andReturn(false);
            this.menu.getTopSelectable.andReturn($("<fiz>"));
            this.menu.getSelectableData.andCallFake(d);
            this.input.getInputValue.andReturn(b.val.slice(0, 1));
            this.view.moveCursor(1);
            expect(this.input.setHint).toHaveBeenCalledWith(b.val);
            function d(e) {
                return (e && e.prop("tagName") === "FIZ") ? b : null
            }
        });
        it("should trigger cursorchange after setting cursor", function () {
            var d = jasmine.createSpy();
            this.$input.on("typeahead:cursorchange", d);
            this.view.moveCursor(1);
            expect(d).toHaveBeenCalled()
        })
    });

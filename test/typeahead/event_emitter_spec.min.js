describe("EventEmitter", function () {
    ...
    it("#on should take the context a callback should be called in", function () {
        var d = {val: 3}, e;
        this.target.onSync("xevent", f, d).trigger("xevent");
        waitsFor(c, "callback was called in the wrong context");
        function f() {
            e = this
        }
    ...
    it("#onAsync callbacks should be invoked asynchronously", function () {
        ...
        waitsFor(b(this.spy, 1), "the callback was not invoked")
    });
    ...
    it("methods should accept multiple event types", function () {
        ...
        waitsFor(b(this.spy, 4), "the callback was not invoked")
    });
    it("the event type should be passed to the callback", function () {
        ...
        waitsFor(a(this.spy, 0, ["sync"]), "bad args");
        waitsFor(a(this.spy, 1, ["async"]), "bad args")
    });
    it("arbitrary args should be passed to the callback", function () {
        ...
        waitsFor(a(this.spy, 0, ["event", 1, 2]), "bad args");
        waitsFor(a(this.spy, 1, ["event", 1, 2]), "bad args")
    });
    it("callback execution should be cancellable", function () {
        var d = jasmine.createSpy().andCallFake(c);
        this.target.onSync("one", d).onSync("one", this.spy).onAsync("two", d).onAsync("two", this.spy).onSync("three", d).onAsync("three", this.spy).trigger("one two three");
        waitsFor(b(d, 3));
        waitsFor(b(this.spy, 0));
        function c() {
            return false
        }
    });
    function b(c, d) {
        return function () {
            return c.callCount === d
        }
    }

    function a(d, c, e) {
        return function () {
            var f = jasmine.getEnv(), g = d.calls[c] ? d.calls[c].args : undefined;
            return f.equals_(g, e)
        }
    }
});
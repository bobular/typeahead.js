describe("Transport", function () {
    ...
    afterEach(function () {
        ...
        function a(b, c) {
            c.readyState !== 4 && c.response(fixtures.ajaxResps.ok)
        }
    });
    it("should use jQuery.ajax as the default transport mechanism", function () {
        var b, c = fixtures.ajaxResps.ok, a = jasmine.createSpy();
        this.transport.get("/test", a);
        b = mostRecentAjaxRequest();
        b.response(c);
        expect(b.url).toBe("/test");
        expect(a).toHaveBeenCalledWith(null, c.parsed)
    });
    it("should respect maxPendingRequests configuration", function () {
        for (var a = 0; a < 10; a++) {
            this.transport.get("/test" + a, $.noop)
        }
        expect(ajaxRequests.length).toBe(6)
    });
    it("should support rate limiting", function () {
        this.transport = new Transport({transport: $.ajax, limiter: a});
        for (var b = 0; b < 5; b++) {
            this.transport.get("/test" + b, $.noop)
        }
        jasmine.Clock.tick(100);
        expect(ajaxRequests.length).toBe(1);
        function a(c) {
            return _.debounce(c, 20)
        }
    });
    it("should cache most recent requests", function () {
        var b = jasmine.createSpy(), a = jasmine.createSpy();
        ...
        this.transport.get("/test1", b);
        this.transport.get("/test2", a);
        jasmine.Clock.tick(0);
        expect(ajaxRequests.length).toBe(2);
        expect(b).toHaveBeenCalledWith(null, fixtures.ajaxResps.ok.parsed);
        expect(a).toHaveBeenCalledWith(null, fixtures.ajaxResps.ok1.parsed)
    });
    ...
    it("should prevent dog pile", function () {
        ...
        runs(function () {
            expect(b).toHaveBeenCalledWith(null, fixtures.ajaxResps.ok.parsed);
            expect(a).toHaveBeenCalledWith(null, fixtures.ajaxResps.ok.parsed)
        })
    });
    it("should always make a request for the last call to #get", function () {
        ...
        this.transport.get("/test" + a, b);
        expect(ajaxRequests.length).toBe(6);
        _.each(ajaxRequests, function (c) {
            c.response(fixtures.ajaxResps.ok)
        });
    ...
    it("should invoke the callback with err set to true on failure", function () {
        var b, c = fixtures.ajaxResps.err, a = jasmine.createSpy();
        this.transport.get("/test", a);
        b = mostRecentAjaxRequest();
        b.response(c);
        expect(b.url).toBe("/test");

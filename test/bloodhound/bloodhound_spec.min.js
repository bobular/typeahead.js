describe("Bloodhound", function () {
    function c(d) {
        return new Bloodhound(_.mixin({datumTokenizer: a, queryTokenizer: b}, d || {}))
    }
    ...
    describe("#initialize", function () {
        beforeEach(function () {
            this.bloodhound = c({initialize: false});
        ...
        it("should not support reinitialization by default", function () {
            var e, d;
            e = this.bloodhound.initialize();
            d = this.bloodhound.initialize();
            expect(e).toBe(d);
        ...
        it("should reinitialize if reintialize flag is true", function () {
            var e, d;
            e = this.bloodhound.initialize();
            d = this.bloodhound.initialize(true);
            expect(e).not.toBe(d);
            expect(this.bloodhound._initialize.callCount).toBe(2)
        });
        it("should clear the index", function () {
            this.bloodhound = c({initialize: false, prefetch: "/prefetch"});
        ...
        it("should load data from prefetch network as fallback", function () {
            ...
            this.bloodhound.prefetch.fromNetwork.andCallFake(d);
            this.bloodhound.initialize();
            expect(this.bloodhound.all()).toEqual(fixtures.data.simple);
            function d(e) {
                e(null, fixtures.data.simple)
            }
        });
        ...
        it("should add local after prefetch is loaded", function () {
            this.bloodhound = c({initialize: false, local: [{foo: "bar"}], prefetch: "/prefetch"});
            this.bloodhound.prefetch.fromNetwork.andCallFake(d);
    ...
    describe("#add", function () {
        it("should add datums to search index", function () {
            ...
            this.bloodhound = c().add(fixtures.data.simple);
            this.bloodhound.search("big", d);
            expect(d).toHaveBeenCalledWith([{value: "big"}, {value: "bigger"}, {value: "biggest"}])
        })
    });
    describe("#get", function () {
        ...
        it("should support array signature", function () {
            expect(this.bloodhound.get(["big", "bigger"])).toEqual([{value: "big"}, {value: "bigger"}])
        });
        it("should support splat signature", function () {
            expect(this.bloodhound.get("big", "bigger")).toEqual([{value: "big"}, {value: "bigger"}])
        });
        it("should return nothing for unknown ids", function () {
            expect(this.bloodhound.get("big", "foo", "bigger")).toEqual([{value: "big"}, {value: "bigger"}])
        })
    });
    describe("#clear", function () {
        it("should remove all datums to search index", function () {
            var d = jasmine.createSpy();
            this.bloodhound = c({local: fixtures.data.simple}).clear();
            this.bloodhound.search("big", d);
    ...
    describe("#clearPrefetchCache", function () {
        it("should clear persistent storage", function () {
            this.bloodhound = c({prefetch: "/prefetch"}).clearPrefetchCache();
    ...
    describe("#clearRemoteCache", function () {
        it("should clear remote request cache", function () {
            ...
            this.bloodhound = c({remote: "/remote"}).clearRemoteCache();
            expect(Transport.resetCache).toHaveBeenCalled()
        })
    });
    describe("#all", function () {
        it("should return all local results", function () {
            this.bloodhound = c({local: fixtures.data.simple});
            expect(this.bloodhound.all()).toEqual(fixtures.data.simple)
        })
    });
    ...
    describe("#search – remote", function () {
        it("should return async matches", function () {
            ...
            this.bloodhound = c({remote: "/remote"});
            this.bloodhound.remote.get.andCallFake(d);
            this.bloodhound.search("dog", $.noop, e);
            expect(e.callCount).toBe(1);
            function d(g, f) {
                f(fixtures.data.animals)
            }
        })
    });
    describe("#search – integration", function () {
        it("should backfill when local/prefetch is not sufficient", function () {
            ...
            f = jasmine.createSpy();
            e = jasmine.createSpy();
            this.bloodhound = c({sufficient: 3, local: fixtures.data.simple, remote: "/remote"});
            this.bloodhound.remote.get.andCallFake(d);
            this.bloodhound.search("big", f, e);
            expect(f).toHaveBeenCalledWith([{value: "big"}, {value: "bigger"}, {value: "biggest"}]);
            expect(e).not.toHaveBeenCalled();
            this.bloodhound.search("bigg", f, e);
            expect(f).toHaveBeenCalledWith([{value: "bigger"}, {value: "biggest"}]);
            expect(e).toHaveBeenCalledWith(fixtures.data.animals);
            function d(h, g) {
                g(fixtures.data.animals)
            }
        });
        it("should remove duplicates from backfill", function () {
            ...
            this.bloodhound = c({
                identify: function (g) {
                    return g.value
                }, local: fixtures.data.animals, remote: "/remote"
            });
            this.bloodhound.remote.get.andCallFake(d);
            this.bloodhound.search("dog", f, e);
            expect(f).toHaveBeenCalledWith([{value: "dog"}]);
            expect(e).toHaveBeenCalledWith([{value: "cat"}, {value: "moose"}]);
            function d(h, g) {
                g(fixtures.data.animals)
            }
        })
    ...
    function a(e) {
        return $.trim(e.value).split(/\s+/)
    }

    function b(d) {
        return $.trim(d).split(/\s+/)
    }
});